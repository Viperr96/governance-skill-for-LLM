...
| `fix-verbose-output` | Article 2, end to end | Inject the article's exact setup into a project in one run: delete vague brevity lines, add the Reply shape rule and the output style (selected via `outputStyle`); ask once, on first install, whether to add the length-gating Stop hook; optionally build the plugin bundle. Cumulative levels, idempotent, every removed line listed |
